    ...
    const result = await client.query(
      `
        ...
        SELECT
          ...
          challenger.players challenger_players,
          challenger.position challenger_position,
          challenger.elo challenger_elo

        FROM challenges ch

        JOIN pair_info challenger
          ON challenger.pair_id = ch.challenger_pair_id
    ...
    const result = await client.query(
      `
        ...
        SELECT
          ...
          CASE
            WHEN ch.challenger_pair_id = $1
              THEN 'challenger'
            ELSE 'challenged'
          END role,

          opponent.pair_id opponent_pair_id,
        ...
        JOIN pair_info opponent
          ON opponent.pair_id =
            CASE
              WHEN ch.challenger_pair_id = $1
                THEN ch.challenged_pair_id
              ELSE ch.challenger_pair_id
            END
...
export async function acceptPendingChallenge(userId, challengeId) {
  const client = await pool.connect();
