...
import {dispatchWhatsAppOutbox,queueUserNotification} from './notifications.js';
...
export async function resolveDispute(adminId,assignmentId,{action,versionId=null}){const client=await pool.connect();try{await client.query('BEGIN');const a=(await client.query(`SELECT * FROM wheel_assignments WHERE id=$1 AND status='disputed' FOR UPDATE`,[assignmentId])).rows[0];if(!a)throw problem('Disputa inexistente');if(action==='void'){await closeAssignment(client,a,'void','admin_void');}else if(action==='select'){const v=(await client.query(`SELECT * FROM wheel_result_versions WHERE id=$1 AND assignment_id=$2`,[versionId,a.id])).rows[0];if(!v)throw problem('Versión inválida');await applySportingResult(client,{assignment:a,winnerPairId:v.winner_pair_id,resultType:v.result_type,score:v.score,playedAt:v.played_at,source:'admin_dispute'});await closeAssignment(client,a,'confirmed','admin_dispute');}else throw problem('Acción inválida');await client.query(`INSERT INTO admin_audit_events(admin_user_id,action,target_type,target_id,data) VALUES($1,'dispute_resolve','assignment',$2,$3::jsonb)`,[adminId,a.id,JSON.stringify({action,versionId})]);await client.query('COMMIT');return {ok:true};}catch(e){await client.query('ROLLBACK');throw e;}finally{client.release();}}
...
export function verifyTotp(secret,code){if(!secret)return true;const key=b32(secret);const now=Math.floor(Date.now()/1000/30);for(let drift=-1;drift<=1;drift++){const buf=Buffer.alloc(8);buf.writeBigUInt64BE(BigInt(now+drift));const h=crypto.createHmac('sha1',key).update(buf).digest();const off=h[h.length-1]&15;const n=((h[off]&0x7f)<<24)|((h[off+1]&255)<<16)|((h[off+2]&255)<<8)|(h[off+3]&255);if(String(n%1000000).padStart(6,'0')===String(code||''))return true;}return false;}
